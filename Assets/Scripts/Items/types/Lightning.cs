	...
	private void drawLine(Vector3 fromPos, Vector3 direction, int reflectionsRemaining) {
		...
		if(Physics.Raycast(ray, out hit, maxStepDistance)) {
			...
			if(hit.collider.GetComponent<Player>()) {
				...
				foreach (GameObject gos in gos1)
        		{
            		gos.transform.rotation = Quaternion.Euler(0, 272, 0);
					gos.transform.position = new Vector3(-3.873f, 5.9499f, -44.13f);
        		}
				
				return;
			}
			else if (!hit.collider.GetComponent<Mirror>()){
				addPoint(fromPos);
